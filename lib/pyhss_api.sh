...
# --- Üst seviye: MSISDN güncelle ---
pyhss_msisdn_guncelle() {
  local imsi=$1 msisdn=$2
  c_info "pyHSS tarafında $imsi aranıyor (jq gerektirir)..."
  if ! command -v jq >/dev/null 2>&1; then
    c_err "jq kurulu değil, pyHSS kayıtları otomatik güncellenemez."
    return 1
  fi

  local s_list s_id is_list is_id auc_id dapn alist

  s_list=$(_pyhss_get "/subscriber/list" 2>/dev/null || _pyhss_get "/subscriber/")
  s_id=$(printf '%s' "$s_list" | jq -r ".[] | select(.imsi==\"$imsi\") | .subscriber_id" 2>/dev/null | head -1)
  auc_id=$(printf '%s' "$s_list" | jq -r ".[] | select(.imsi==\"$imsi\") | .auc_id" 2>/dev/null | head -1)
  dapn=$(printf '%s' "$s_list" | jq -r ".[] | select(.imsi==\"$imsi\") | .default_apn" 2>/dev/null | head -1)
  alist=$(printf '%s' "$s_list" | jq -r ".[] | select(.imsi==\"$imsi\") | .apn_list" 2>/dev/null | head -1)
  ...
  if [ -n "$auc_id" ]; then
    pyhss_subscriber_ekle "$imsi" "$auc_id" "$dapn" "$alist" "$msisdn" >/dev/null || return 1
    pyhss_ims_subscriber_ekle "$imsi" "$msisdn" >/dev/null || return 1
    c_ok "pyHSS MSISDN başarıyla güncellendi (Kayıt yenilendi)."
  else
    c_err "pyHSS Subscriber (auc_id) bulunamadı, MSISDN güncellenemedi."
    return 1
  fi
}
