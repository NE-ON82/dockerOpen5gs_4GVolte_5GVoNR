...
webui_msisdn_guncelle() {
  local imsi=$1 msisdn=$2
  local js="db.subscribers.updateOne({imsi: \"$imsi\"}, {\$set: {msisdn: [\"$msisdn\"]}})"
  if _mongo_eval "$js" | grep -i "acknowledged" >/dev/null; then
    c_ok "EPC (WebUI) abonesi MSISDN güncellendi: $msisdn"
    return 0
  else
    c_err "EPC MSISDN güncellenemedi."
    return 1
  fi
}
